    ...
    def Attack(self, Target):
        ...
        if hit:
            if parry:
                if choiced == 1:
                    sw.print_slow(
                        f"{self.name} attacked {Target.name} but {Target.name} parried the attack!"
                    )
                    damage = self.basicATK // 2
                elif choiced == 2:
                    sw.print_slow(
                        f"{self.name} used {self.move1_name} on {Target.name} but {Target.name} parried the attack"
                    )
                    damage = self.Move1 // 2
                elif choiced == 3:
                    sw.print_slow(
                        f"{self.name} used {self.move2_name} on {Target.name} but {Target.name} parried the attack"
                    )
                    damage = self.Move2 // 2
                elif choiced == 4:
                    sw.print_slow(
                        f"{self.name} used {self.move3_name} on {Target.name} but {Target.name} parried the attack"
                    )
                    damage = self.Move3 // 2
            elif critHit:
                if choiced == 1:
